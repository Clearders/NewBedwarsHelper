...
public final class TrajectoryPredictionSimulator {
    ...
    private static void addThrowablePrediction(
        ...
        if (path != null) {
            simulate(level, player, projectile, movement, false, 0, start, config, path);
        }
    }
    ...
    private static void addBowPrediction(
        ...
        if (path != null) {
            simulate(level, player, arrow, movement, true, 0, start, config, path);
        }
    }
    ...
    private static void addCrossbowPredictions(
        ...
        Vec3 start = launchPosition(player, partialTicks);
        int piercingLevel = getPiercingLevel(level, crossbow);
        ...
        for (int index = 0; index < projectileCount && FRAME.pathCount() < MAX_PATHS; index++) {
            ...
            Path path = FRAME.nextPath(true);
            if (path != null) {
                simulate(level, player, arrow, movement, true, piercingLevel, start, config, path);
            }
        }
    }
    ...
    private static void simulate(
        ...
        for (int tick = 1; tick <= MAX_PREDICTION_TICKS; tick++) {
            // ProjectileUtil derives the entity hitbox margin from Entity#tickCount.
            // The prediction uses a dummy entity and does not call Entity#tick, so keep
            // its age aligned with the start of the corresponding vanilla tick.
            projectile.tickCount = tick - 1;
            ...
            if (arrow) {
                ...
                HitResult hitResult = findArrowHit(level, projectile, from, step.to, collisionFilter, piercingLevel);
                ...
                collisionFilter.leftOwner = leftOwner;
                prepareStep(step, from, movement, travelled, maxLength);
                HitResult hitResult = findThrowableHit(level, projectile, from, step.to, collisionFilter);
                Vec3 to = hitResult.getType() == HitResult.Type.MISS ? step.to : hitResult.getLocation();
                double tickFraction = movement.lengthSqr() == 0.0D ? 0.0D : Math.min(1.0D, from.distanceTo(to) / movement.length());
                if (!path.appendSegment(from, to, precision)) {
                    return;
    ...
    private static HitResult findThrowableHit(
        ...
        EntityHitResult entityHit = ProjectileUtil.getEntityHitResult(
                level, projectile, from, entityEnd, searchBox, collisionFilter
        );
    ...
    private static HitResult findArrowHit(
        ...
        // A piercing level of N lets the arrow pass through N entities and stops it
        // at the next one. If this sweep contains no terminal entity, vanilla keeps
        // stepping toward the already-computed block/world-border result.
        return sortedHits.size() > piercingLevel ? sortedHits.get(piercingLevel) : blockHit;
    }
    ...
    private static int getPiercingLevel(Level level, ItemStack weapon) {
        var enchantments = level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT);
        var piercing = enchantments.getOrThrow(Enchantments.PIERCING);
        return Math.max(0, EnchantmentHelper.getItemEnchantmentLevel(piercing, weapon));
    }
